...
def test_get_api_token_with_valid_key(api_key = API_key):
    ...
    status, result = mc.get_api_token(api_key)
...
def test_get_api_token_with_no_valid_key(api_key = NV_API_key):
...
def test_get_managers_list(api_key = API_key):
    ...
    _, result = mc.get_api_token(api_key)
    ...
    status, result = mc.get_managers_list(API_token)

    # Сверяем полученные данные с нашими ожиданиями
    assert status == 200
    assert len(result) > 0

def test_delete_with_valid_api_token(api_key = API_key):
    ...
    status, _ = mc.delete_api_token(API_token)

    # Сверяем полученные данные с нашими ожиданиями
    assert status == 204

def test_delete_with_no_valid_api_token(api_key = API_key):
    ...
    # Отправляем запрос на удаление API токена и сохраняем полученный ответ с кодом статуса в status,
    # а текст ответа в result
    status, result = mc.delete_api_token(API_token)
...
def test_add_new_manager_with_valid_data(api_key = API_key, name="Иванова Мария Александровна", phone="79681234576",
    ...
    status, result = mm.add_new_manager(API_token, name, phone, email, filials, salaryFilialId, roles, enabled,
...
def test_add_new_manager_with_valid_data_2(api_key = API_key, name="Иванова Мария Александровна", phone="79681234576",
                                         email="ivanova@site.com", filials=[31539], salaryFilialId=0, roles=[68186],
...
def test_add_new_manager_with_no_valid_data(api_key = API_key, name="Иванова Мария Александровна",
...
def test_update_manager_with_valid_data(api_key = API_key, name="Петров Петр Михайлович",
    ...
    # в result
    _, result = mc.get_managers_list(API_token)
    ...
    if len(result) > 1:
        ...
        status, result = mm.update_manager(API_token, managerID, name, phone, enabled, filials, roles, color,
...
def test_update_manager_with_no_valid_parameter(api_key = API_key, name="Петров Петр Михайлович",
                        phone="79154752160", enabled=False, filials=[31539], roles=[68186],
                        color="#222fff", salaryFilialId=0, email="petrov@mail.com", replaceToManagerId=0,
                        password="11111111", additionalContacts="", isStaff=True, isWork=True, sendNotifies=True,
                        startDate=None, endDate=None, contractNumber="", contractDate=None,
    ...
    while True:
        ...
        if NV_managerID not in id_array:
            ...
            if len(result) > 1:
                # Обновляем данные сотрудника
                status, result = mm.update_manager(API_token, NV_managerID, name, phone, enabled, filials, roles, color,
                                                  salaryFilialId, email, replaceToManagerId, password, additionalContacts,
                                                  isStaff, isWork, sendNotifies, startDate, endDate, contractNumber,
                                                  contractDate, birthDate, passportData, comment, rateId, isOwner)
                # Сверяем полученный ответ с ожидаемым результатом
